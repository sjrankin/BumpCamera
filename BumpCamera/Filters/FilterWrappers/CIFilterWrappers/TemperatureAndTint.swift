...
class TemperatureAndTint: FilterParent, Renderer
{
    static let _ID: UUID = UUID(uuidString: "ae7c6800-45d0-454b-a339-67343685b9b2")!
    
    func ID() -> UUID
    {
        return TemperatureAndTint._ID
    }
    ...
    func Title() -> String
    {
        return TemperatureAndTint.Title()
    }
    ...
    var Description: String = "Temp & Tint"
    
    var IconName: String = "Temp & Tint"
    ...
    {
        Reset("TemperatureAndTint.Initialize")
        ...
        Context = CIContext()
        PrimaryFilter = CIFilter(name: "CITemperatureAndTint")
        Initialized = true
    }
    ...
    func Render(PixelBuffer: CVPixelBuffer) -> CVPixelBuffer?
    {
        ...
        let SourcePoint = ParameterManager.GetVector(From: ID(), Field: .SourcePoint, Default: CIVector(x: 0, y: 0))
        let TargetPoint = ParameterManager.GetVector(From: ID(), Field: .TargetPoint, Default: CIVector(x: 0, y: 0))
        let LocateSource = ParameterManager.GetBool(From: ID(), Field: .AutoLocateSource, Default: true)
        let LocateTarget = ParameterManager.GetBool(From: ID(), Field: .AutoLocateTarget, Default: true)
        
        PrimaryFilter.setValue(SourcePoint, forKey: "inputNeutral")
        PrimaryFilter.setValue(TargetPoint, forKey: "InputTargetNeutral")
        ...
        {
            print("Allocation failure in TemperatureAndTint.")
            return nil
        }
    ...
    func DefaultFieldValue(Field: FilterManager.InputFields) -> (FilterManager.InputTypes, Any?)
    {
        ...
        case .AutoLocateTarget:
            return (.BoolType, true as Any?)
            
        case .AutoLocateSource:
            return (.BoolType, true as Any?)
    ...
    {
        return TemperatureAndTint.SupportedFields()
    }
    
    public static func SupportedFields() -> [FilterManager.InputFields]
    {
        var Fields = [FilterManager.InputFields]()
        Fields.append(.SourcePoint)
        Fields.append(.TargetPoint)
        Fields.append(.AutoLocateSource)
        Fields.append(.AutoLocateTarget)
    ...
    {
        return TemperatureAndTint.SettingsStoryboard()
    }
    
    public static func SettingsStoryboard() -> String?
    {
        return "TemperatureAndTintSettingsUI"
    }
    ...
    {
        return TemperatureAndTint.FilterTarget()
    }
    ...
    {
        var Keywords = [String]()
        Keywords.append("Filter: CITemperatureAndTint")
    ...
    var FilterKernel: FilterManager.FilterKernelTypes
    {
        get
        {
            return TemperatureAndTint.FilterKernel
        }
    }
    ...
    {
        return TemperatureAndTint.Ports()
    }
}
