...
class CornerGradient: FilterParent, Renderer
{
    required override init()
    {
        let DefaultLibrary = MetalDevice?.makeDefaultLibrary()
        let KernelFunction = DefaultLibrary?.makeFunction(name: "CornerGradient")
        do
        {
    ...
    static let _ID: UUID = UUID(uuidString: "101e1d02-c028-4b88-b090-999c3c37c9c0")!
    ...
    var Description: String = "CornerGradient"
    
    var IconName: String = "CornerGradient"
    ...
    func Initialize(With FormatDescription: CMFormatDescription, BufferCountHint: Int)
    {
        ...
        {
            fatalError("Unable to allocation texture cache in CornerGradient.")
        }
        else
        {
    ...
    func Render(PixelBuffer: CVPixelBuffer) -> CVPixelBuffer?
    {
        ...
        {
            print("Allocation failure for new pixel buffer pool in CornerGradient.")
            return nil
        }
        ...
        {
            print("Error creating textures in CornerGradient.")
            return nil
        }
        ...
        CommandEncoder.label = "CornerGradient Generator"
    ...
    func Render(Image: UIImage) -> UIImage?
    {
        ...
        {
            print("Error creating input texture in CornerGradient.Render.")
            return nil
        }
    ...
    func Render(Image: CIImage) -> CIImage?
    {
        ...
        if let IFinal = Render(Image: UImage)
        {
            ...
            {
                print("Error converting UIImage to CIImage in CornerGradient.Render(CIImage)")
                return nil
            }
        }
        else
        {
            print("Error returned from Render(UIImage) in CornerGradient.Render(CIImage)")
            return nil
        }
    }
    ...
    func DefaultFieldValue(Field: FilterManager.InputFields) -> (FilterManager.InputTypes, Any?)
    {
        ...
        case .ULColor:
            return (.ColorType, UIColor.cyan as Any?)
            
        case .URColor:
            return (.ColorType, UIColor.magenta as Any?)
    ...
    public static func SupportedFields() -> [FilterManager.InputFields]
    {
        ...
        Fields.append(.HasULColor)
        Fields.append(.HasURColor)
        Fields.append(.HasLLColor)
        Fields.append(.HasLRColor)
        Fields.append(.AlphaGradiates)
        Fields.append(.ULColor)
        Fields.append(.URColor)
        Fields.append(.LLColor)
        Fields.append(.LRColor)
    ...
    {
        var Keywords = [String]()
        Keywords.append("Filter: CornerGradient")
