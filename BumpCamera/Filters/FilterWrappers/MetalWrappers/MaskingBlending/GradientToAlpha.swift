...
class GradientToAlpha: FilterParent, Renderer
{
    ...
    static let _ID: UUID = UUID(uuidString: "79e98f88-b0df-4c75-bf52-fe29b38a7e3c")!
    ...
    var Description: String = "GradientToAlpha"
    ...
    {
        Reset("GradientToAlpha.Initialize")
    ...
    func Render(PixelBuffer: CVPixelBuffer, AltSettings: FilterSettingsBlob) -> CVPixelBuffer?
    {
        ...
        var GradientDescription = ParameterManager.GetString(Blob: AltSettings, Field: .ColorMapGradient, Default: "(White)@(0.0),(Black)@(1.0)")
        if GradientDescription.isEmpty
        {
            GradientDescription = "(White)@(0.0),(Black)@(1.0)"
        }
        let IsVertical = ParameterManager.GetInt(Blob: AltSettings, Field: .GradientOrientation, Default: 0) == 1
    ...
    func Render(Image: UIImage, AltSettings: FilterSettingsBlob) -> UIImage?
    {
        ...
        {
            print("Error creating input texture in GradientToAlpha.Render.")
            return nil
        }
        ...
        let IsVertical = ParameterManager.GetInt(Blob: AltSettings, Field: .GradientOrientation, Default: 0) == 1
        let GradientImage = GradientManager.CreateGradientImage(From: GradientDescription,
                                                                WithFrame: CGRect(x: 0, y: 0, width: ImageWidth, height: ImageHeight),
                                                                IsVertical: IsVertical, ReverseColors: false)
        let GradientPixelBuffer = GetPixelBufferFrom(GradientImage)
        let GradientTexture = MakeTextureFromCVPixelBuffer(PixelBuffer: GradientPixelBuffer!, TextureFormat: .bgra8Unorm)
    ...
    func Render(Image: CIImage) -> CIImage?
    {
        ...
        if let IFinal = Render(Image: UImage)
        {
            ...
            {
                print("Error converting UIImage to CIImage in GradientToAlpha.Render(CIImage)")
                return nil
            }
        }
        else
        {
            print("Error returned from Render(UIImage) in GradientToAlpha.Render(CIImage)")
            return nil
        }
    }
    ...
    {
        return [.ColorMapGradient, .GradientOrientation,
    ...
    {
        var Keywords = [String]()
        Keywords.append("Filter: GradientToAlpha")
