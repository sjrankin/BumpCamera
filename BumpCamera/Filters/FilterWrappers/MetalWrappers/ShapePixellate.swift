...
class ShapePixellate: FilterParent, Renderer
{
    required override init()
    {
        let DefaultLibrary = MetalDevice?.makeDefaultLibrary()
        let KernelFunction = DefaultLibrary?.makeFunction(name: "ShapePixellate")
        do
        {
    ...
    static let _ID: UUID = UUID(uuidString: "0b868a7a-a9cd-4410-8c28-bdf4a1a059aa")!
    ...
    var Description: String = "ShapePixellate"
    ...
    func Initialize(With FormatDescription: CMFormatDescription, BufferCountHint: Int)
    {
        ...
        {
            fatalError("Unable to allocation texture cache in ShapePixellate.")
        }
        else
        {
    ...
    func Render(PixelBuffer: CVPixelBuffer) -> CVPixelBuffer?
    {
        ...
        if !Initialized
        {
            fatalError("ShapePixellate not initialized at Render(CVPixelBuffer) call.")
        }
        ...
        let PWidth = ParameterManager.GetInt(From: ID(), Field: .PixellateWidth, Default: 32)
        let PHeight = ParameterManager.GetInt(From: ID(), Field: .PixellateHeight, Default: 32)
        let Shape = ParameterManager.GetInt(From: ID(), Field: .PixellateShape, Default: 0)
        let BlockColorDet = ParameterManager.GetInt(From: ID(), Field: .BlockColorDetermination, Default: 0)
        let BlockColor = ParameterManager.GetColor(From: ID(), Field: .BlockColor, Default: UIColor.red)
        let DrawOutline = ParameterManager.GetBool(From: ID(), Field: .DrawPixelOutline, Default: true)
        let OutlineColorDet = ParameterManager.GetInt(From: ID(), Field: .PixelOutlineColorDetermination, Default: 0)
        let OutlineColor = ParameterManager.GetColor(From: ID(), Field: .PixelOutlineColor, Default: UIColor.black)
        let OutlineThickness = ParameterManager.GetInt(From: ID(), Field: .PixelOutlineThickness, Default: 2)
        let Parameter = ShapePixellateParameters(Size: simd_int1(PWidth),
                                                 Shape: simd_int1(Shape),
                                                 BlockColorFrom: simd_int1(BlockColorDet),
                                                 BlockColor: BlockColor.ToFloat4(),
                                                 DrawOutline: simd_bool(DrawOutline),
                                                 OutlineColorFrom: simd_int1(OutlineColorDet),
                                                 OutlineColor: OutlineColor.ToFloat4(),
                                                 OutlineThickness: simd_int1(OutlineThickness))
        let Parameters = [Parameter]
        ParameterBuffer = MetalDevice!.makeBuffer(length: MemoryLayout<ShapePixellateParameters>.stride, options: [])
        memcpy(ParameterBuffer.contents(), Parameters, MemoryLayout<ShapePixellateParameters>.stride)
        
        var NewPixelBuffer: CVPixelBuffer? = nil
        CVPixelBufferPoolCreatePixelBuffer(kCFAllocatorDefault, BufferPool!, &NewPixelBuffer)
        guard let OutputBuffer = NewPixelBuffer else
        {
            print("Allocation failure for new pixel buffer pool in ShapePixellate.")
            return nil
        }
        ...
        {
            print("Error creating textures in ShapePixellate.")
            return nil
        }
        ...
        CommandEncoder.label = "Shape Pixellator"
    ...
    func Render(Image: UIImage) -> UIImage?
    {
        ...
        {
            print("Error creating input texture in ShapePixellate.Render.")
            return nil
        }
    ...
    func Render(Image: CIImage) -> CIImage?
    {
        ...
        if let IFinal = Render(Image: UImage)
        {
            ...
            {
                print("Error converting UIImage to CIImage in ShapePixellate.Render(CIImage)")
                return nil
            }
        }
        else
        {
            print("Error returned from Render(UIImage) in ShapePixellate.Render(CIImage)")
            return nil
        }
    }
    ...
    func DefaultFieldValue(Field: FilterManager.InputFields) -> (FilterManager.InputTypes, Any?)
    {
        ...
        {
        case .PixellateWidth:
            return (.IntType, 32 as Any?)
            
        case .PixellateHeight:
            return (.IntType, 32 as Any?)
        ...
        case .DrawPixelOutline:
            return (.BoolType, true as Any?)
            
        case .PixelOutlineColorDetermination:
            return (.IntType, 0 as Any?)
            
        case .PixelOutlineColor:
            return (.ColorType, UIColor.black as Any?)
            
        case .PixelOutlineThickness:
    ...
    {
        return [.PixellateWidth, .PixellateHeight, .BlockColorDetermination, .BlockColor,
                .DrawPixelOutline, .PixelOutlineColorDetermination, .PixelOutlineColor,
                .PixelOutlineThickness, .PixellateShape,
    ...
    {
        return "ShapePixellateSettingsUI"
    }
    ...
    {
        var Keywords = [String]()
        Keywords.append("Filter: ShapePixellate")
